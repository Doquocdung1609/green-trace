...
const schema = z.object({
  ...
  phone: z.string()
  .trim()
  .regex(/^0[3-9]\d{8}$/, { message: "Số điện thoại phải bắt đầu bằng 0 và có đúng 10 số (VD: 0967622712)" }),
  address: z.string().min(1, "Nhập địa chỉ giao hàng"),
  lat: z.number().optional(),
  lng: z.number().optional(),
  payment: z.enum(["solana"]),
});
        ...
        <Form {...form}>
          ...
          >
            ...
            render={({ field }) => (
                ...
                <FormControl>
                    {/* Dùng Input ẩn để đảm bảo giá trị luôn là "solana" */}
                    <Input type="hidden" {...field} value="solana" />
                </FormControl>
                <div className="p-4 bg-green-50 dark:bg-green-900/30 rounded-lg text-center">
                    <p className="text-green-700 dark:text-green-300 font-medium">
                    💳 Thanh toán qua Ví Solana (Phantom)
                    </p>
                    <p className="text-sm text-gray-600 dark:text-gray-400 mt-1">
                    Giao dịch sẽ được xử lý trực tiếp trên blockchain
                    </p>
                </div>
                <FormMessage />
                </FormItem>
